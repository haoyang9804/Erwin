...
let result: ast.CompileResult;

function traverse_node(node: ast.ASTNode, depth: number = 0) {
...
try {
    ...
    const writer = new ast.ASTWriter(
        ...
        formatter,
        result.compilerVersion ? result.compilerVersion : ast.LatestCompilerVersion
    );
