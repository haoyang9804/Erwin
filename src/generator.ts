...
class FunctionDeclarationGenerator extends DeclarationGenerator {
  ...
  generate_function_body() : void {
    ...
    funcstat_dag.update(funcstat_dag.newNode(this.irnode!.id), state_mutability_range);
...
class IdentifierGenerator extends LRValueGenerator {
  ...
  generate(cur_expression_complex_level : number) : void {
    ...
    if (irdecl !== undefined) {
      ...
      expr2read_variables.set(this.id, merge_set(expr2read_variables.get(this.id)!, new Set<number>([irdecl.id])));
    }
    if (config.debug) {
      indent -= 2;
