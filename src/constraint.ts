...
import { assert, createCustomSet, extendArrayofMap, pickRandomElement, shuffle, intersection_array, selectRandomElements } from "./utility";
...
export class TypeDominanceDAG {
  ...
  */
  ...
  resolve() : void {
    ...
    if (debug) log.write("1. Finish initializing the resolution");
    // !1. Get this.heads and this.tails
    this.get_heads_and_tails();
    if (debug) log.write("2. Finish initializing this.heads and this.tails");
    ...
    if (debug) log.write("3. Finish initializing node2tail");
    // !3. Map edges to their reachable this.tails
    for (let tail of this.tails) {
      this.dfs4edge2tail(tail, tail);
    }
    if (debug) log.write("4. Finish initializing edge2tail");
    ...
    if (debug) log.write("4. Finish removing removable subtype dominations");
    ...
    if (debug) log.write("5. Finish restricting the type range of heads");
    // !6. Assign types to this.heads
    this.allocate_type_candidates_for_heads();
    if (debug) log.write("6. Finish allocating type candidates for heads");
    if (debug) log.write("7. Start traversing all type resolutions for heads");
    ...
    for (const head_resolve of this.heads2type) {
      ...
      if (debug) log.write("8. Finish allocating type candidates for tails based on the current type resolution for heads");
      ...
      if (debug) log.write("9. Finish building type relations among tails");
      // !10. Resolve the types of tails.
      const plausible_type_resolution_for_tails = this.resolve_tails(tail2types);
      if (debug) log.write("10. Finish resolving types for tails");
      ...
      for (let [head, type_of_head] of head_resolve) {
        ...
        if (debug) log.write("11. Finish checking the compatibility of resolved types of tails with resolved types of heads");
        ...
        if (compatible_with_resolved_tails) {
          ...
          if (debug) log.write("12. Finish resolving types of nonheads and nontails");
        }
        else {
          good_resolve = false;
          break;
