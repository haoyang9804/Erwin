...
type VisMutKind = VarVisKind | FuncVisMutKind;
...
export class VisMutKindProvider {
  ...
  static combine_vis_mut(vis: FuncVis, mut: FuncStat): VisMutKind {
    if (vis.typeName === "FuncInteral") {
      if (mut.typeName === "FuncPure") {
        return FuncVisMutKindProvider.internal_pure();
      } else if (mut.typeName === "FuncView") {
        return FuncVisMutKindProvider.internal_view();
      } else if (mut.typeName === "FuncEmpty") {
        return FuncVisMutKindProvider.internal_empty();
      } else {
        throw new Error("Invalid FuncStat: FuncInternal can only be Pure, View, or Empty");
      }
    }
      ...
      if (mut.typeName === "FuncPure") {
        ...
        return FuncVisMutKindProvider.external_view();
      } else if (mut.typeName === "FuncPayable") {
        return FuncVisMutKindProvider.external_payable();
      } else if (mut.typeName === "FuncEmpty") {
        return FuncVisMutKindProvider.external_empty();
      } else {
        throw new Error("Invalid FuncStat: FuncExternal can only be Pure, View, Payable, or Empty");
      }
    }
      ...
      if (mut.typeName === "FuncPure") {
        ...
        throw new Error("Invalid FuncStat: FuncPublic can only be Pure, View, Payable, or Empty");
      }
    }
      ...
      if (mut.typeName === "FuncPure") {
        ...
        throw new Error("Invalid FuncStat: FuncPrivate can only be Pure, View, or Empty");
      }
    }
    else {
      throw new Error("Invalid FuncVis: FuncVis can only be Internal, External, Public, or Private");
    }
  }
}
...
abstract class FuncVisMut extends VisMut {
  str(): string {
...
abstract class VarVisMut extends VisMut {
  str(): string {
    return this.typeName;
  }
  subs(): VisMut[] {
