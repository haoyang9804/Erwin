...
with open('./test_programs/erwin_bugs.txt') as f:
    bugs_erwin = set([l.split('->')[0].strip() for l in f.read().splitlines()])
with open('./test_programs/fuzzol_bugs.txt') as f:
    bugs_fuzzol = set([l.split('->')[0].strip() for l in f.read().splitlines()])
with open('./test_programs/acf_bugs.txt') as f:
