...
def process_gcov_files(gcov_files):
  ...
  for gcov_file in gcov_files:
    collected_lines, lines_count = process_gcov_file(gcov_file)
    total_collected_lines.update(collected_lines)
    total_lines_count += lines_count

  return total_collected_lines, total_lines_count

def parallel_process_gcov_files(gcov_files):
  with Pool() as pool:
    results = pool.map(process_gcov_file, gcov_files)

  total_collected_lines = set()
  total_lines_count = 0

  for collected_lines, lines_count in results:
    total_collected_lines.update(collected_lines)
    total_lines_count += lines_count
...
def chunk_processor(chunk):
  results = []
  for gcov_file in chunk:
      results.append(process_gcov_file(gcov_file))
  return results

def parallel_process_gcov_files_by_chunk(gcov_files, chunk_size=cpu_count()):
  num_cores = cpu_count()
  chunks = [gcov_files[i:i + chunk_size] for i in range(0, len(gcov_files), chunk_size)]
  
  total_collected_lines = set()
  total_lines_count = 0

  with ProcessPoolExecutor(max_workers=num_cores) as executor:
    future_to_chunk = {executor.submit(chunk_processor, chunk): chunk for chunk in chunks}
    for future in as_completed(future_to_chunk):
      chunk_results = future.result()
      for collected_lines, lines_count in chunk_results:
        total_collected_lines.update(collected_lines)
        total_lines_count += lines_count
...
def extract_collected_lines(gcov_folder_path) -> set :
  # update gcov files
  start = time.time()
  generate_gcov_files_python_in_parallel(gcov_folder_path)
  end = time.time()
  print(Fore.MAGENTA + f"generate_gcov_files_python_in_parallel: {end-start} seconds")
  # find all gcov files
  gcov_files = []
  ...
  # start = time.time()
  # res = parallel_process_gcov_files(gcov_files)
  # end = time.time()
  # print(Fore.MAGENTA + f"parallel_process_gcov_files: {end-start} seconds")
  # start = time.time()
  # res = parallel_process_gcov_files_by_chunk(gcov_files)
  # end = time.time()
  # print(Fore.MAGENTA `+ f"parallel_process_gcov_files_by_chunk: {end-start} seconds")
  return res
...
def generate_gcov_files_linux(gcda_folder_path) :
...
def generate_gcov_files_linux_in_parallel(gcda_folder_path) :
  remove_gcov_files(gcda_folder_path)
  # linux command of finding all gcda files in the folder and using gcov to generate gcov files
  cur_path = os.getcwd()
  os.chdir(gcda_folder_path)
  command = 'find . -name "*.gcda" | parallel llvm-cov gcov'
  try:
    subprocess.run(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
  except Exception as e:
    print(f"Error: {e}")
...
def run_gcov(file, gcda_directory):
  """Run gcov on a single file."""
  cur_dir = os.getcwd()
  os.chdir(gcda_directory)
  try:
    result = subprocess.run(f'llvm-cov gcov {file}', shell=True, capture_output=True, text=True, check=True)
    os.chdir(cur_dir)  
...
def generate_gcov_files_python_in_parallel(gcda_directory, max_workers=None):
  ...
  with ProcessPoolExecutor(max_workers=max_workers) as executor:
    futures = [executor.submit(run_gcov, file, gcda_directory) for file in gcda_files]
